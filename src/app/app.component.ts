    ...
    this.router.events.subscribe((event) => {
      if (event instanceof NavigationEnd) {
        ...
        // this.showAdminSidebar = ['/dash-board-admin', '/farmmanagement', '/user-management' ,'/edit-farm/:id'].some(path => currentUrl.startsWith(path));
          ...
          this.showUserSidebar = ['/dash-board-user', '/user-farm-list', '/crop-health', '/farms', '/farms/:id','/user-profile'].some(path => currentUrl.startsWith(path)) || currentUrl.includes('/farms/');
